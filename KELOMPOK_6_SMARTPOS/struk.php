<?php
// ================================================================
// SMARTPOS - Struk Belanja (dari Checkout langsung)
// File: struk.php
// Deskripsi: Menampilkan struk setelah proses checkout
//            Data diambil dari session (last_order, bayar, metode)
//            Setelah ditampilkan, session struk dihapus
// ================================================================

session_start();
$con = mysqli_connect("localhost", "root", "", "smartpos");
if (!$con) { die("Koneksi database gagal"); }

// Include helper functions
include_once 'config/koneksi.php';

// Cek session struk
if (!isset($_SESSION['last_order']) || !isset($_SESSION['bayar_struk'])) {
    header("location: index.php");
    exit;
}

$id_transaksi = $_SESSION['last_order'];
$bayar        = $_SESSION['bayar_struk'];
$metode       = $_SESSION['metode_struk'] ?? 'Tunai';

// Ambil data kasir
$kasir_name = $_SESSION['nama_lengkap'] ?? 'Kasir';

// Ambil detail barang
$data = mysqli_query($con, "
    SELECT p.namaproduk, dt.harga_saat_itu as harga, dt.qty
    FROM detail_transaksi dt
    JOIN produk p ON dt.idproduk = p.idproduk
    WHERE dt.id_transaksi = '$id_transaksi'
");

$total = 0;
$items = [];
while ($d = mysqli_fetch_array($data)) {
    $subtotal = $d['harga'] * $d['qty'];
    $total += $subtotal;
    $items[] = $d;
}

$kembalian = $bayar - $total;

// Hapus session struk (agar tidak bisa diakses ulang)
unset($_SESSION['last_order']);
unset($_SESSION['bayar_struk']);
unset($_SESSION['metode_struk']);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Struk Belanja - SMARTPOS</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { 
            font-family: 'Courier New', Courier, monospace; 
            font-size: 11px; 
            margin: 0; 
            padding: 10px; 
            background: #eee; 
        }
        .struk { 
            width: 280px; 
            background: #fff; 
            padding: 12px; 
            margin: 0 auto; 
            box-shadow: 0 0 10px rgba(0,0,0,0.15); 
        }
        .center { text-align: center; }
        .flex { display: flex; justify-content: space-between; margin: 2px 0; }
        .line { border-bottom: 1px dashed #000; margin: 6px 0; }
        .store-name { font-size: 16px; font-weight: bold; margin: 0; }
        .store-sub { font-size: 10px; margin: 2px 0; color: #333; }
        .trx-id { font-size: 11px; font-weight: bold; margin: 2px 0; }
        .item-name { font-size: 10px; margin-top: 3px; }
        .item-detail { font-size: 10px; }
        .total-label { font-size: 13px; font-weight: bold; }
        .footer-text { font-size: 9px; margin-top: 8px; line-height: 1.4; }
        .footer-note { font-size: 8px; color: #555; margin-top: 6px; line-height: 1.3; }
        .footer-time { font-size: 8px; color: #888; margin-top: 4px; }
        .btn-print { 
            display: block; width: 100%; padding: 10px; margin-top: 10px; 
            cursor: pointer; text-align: center; background: #0d6efd; 
            color: white; text-decoration: none; border: none; border-radius: 5px;
            font-size: 13px;
        }
        
        /* Print - struk muat 1 halaman */
        @media print { 
            @page { 
                size: 58mm auto;
                margin: 0; 
            }
            body { 
                background: #fff; 
                margin: 0; 
                padding: 2mm;
            } 
            .btn-print, .btn-container { display: none !important; }
            .struk { 
                box-shadow: none; 
                margin: 0; 
                width: 100%; 
                padding: 0;
                page-break-inside: avoid;
            } 
        }
    </style>
</head>
<body onload="window.print()">

<div class="struk">
    <div class="center">
        <p class="store-name">SMARTPOS</p>
        <p class="store-sub">Sistem Kasir Digital</p>
        <p class="trx-id"><?= formatIdTransaksi($id_transaksi); ?></p>
    </div>

    <div class="line"></div>

    <div class="flex">
        <span>Tanggal</span>
        <span><?= date('d/m/Y H:i'); ?></span>
    </div>
    <div class="flex">
        <span>Kasir</span>
        <span><?= htmlspecialchars($kasir_name); ?></span>
    </div>

    <div class="line"></div>

    <?php foreach ($items as $i): ?>
        <div class="item-name"><?= htmlspecialchars($i['namaproduk']); ?></div>
        <div class="flex item-detail">
            <span><?= $i['qty']; ?> x <?= number_format($i['harga']); ?></span>
            <span><?= number_format($i['qty'] * $i['harga']); ?></span>
        </div>
    <?php endforeach; ?>

    <div class="line"></div>

    <div class="flex total-label">
        <span>TOTAL</span>
        <span>Rp <?= number_format($total); ?></span>
    </div>

    <div class="line"></div>

    <div class="flex">
        <span>Metode Bayar</span>
        <span><?= htmlspecialchars($metode); ?></span>
    </div>
    <div class="flex">
        <span>Uang Diterima</span>
        <span>Rp <?= number_format($bayar); ?></span>
    </div>
    
    <?php if ($metode == 'Tunai' && $kembalian > 0): ?>
    <div class="flex" style="font-weight:bold;">
        <span>KEMBALI</span>
        <span>Rp <?= number_format($kembalian); ?></span>
    </div>
    <?php endif; ?>

    <div class="line"></div>

    <div class="center">
        <p class="footer-text">Terima Kasih telah berbelanja!<br>
        Kami nantikan kedatangan Anda kembali.</p>
        
        <p class="footer-note">Barang yang sudah dibeli<br>tidak dapat ditukar/dikembalikan.</p>
        
        <p class="footer-time"><?= date('d/m/Y H:i'); ?> WIB</p>
    </div>
</div>

<div class="btn-container center" style="width:280px; margin:0 auto;">
    <a href="transaksi.php" class="btn-print">&laquo; Kembali ke Kasir</a>
</div>

</body>
</html>